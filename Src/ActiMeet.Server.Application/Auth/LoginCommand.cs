...
namespace ActiMeet.Server.Application.Auth;
	...
	IJwtProvider jwtProvider) : IRequestHandler<LoginCommand, Result<LoginCommandResponse>>
{
	public async Task<Result<LoginCommandResponse>> Handle(LoginCommand request, CancellationToken cancellationToken)
	{
		...
		if (user is null) return Result<LoginCommandResponse>.Failure("Kullanıcı bulunamadı.");
		...
		if (signInResult.IsNotAllowed) return (500, "Mail adresiniz onaylı değil");

		if (!signInResult.Succeeded) return (500, "Şifreniz yanlış");
